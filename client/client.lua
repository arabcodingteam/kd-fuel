	...
	while true do
		...
		if pumpDistance < 2.5 then
			...
			if haveNozzle == true then
				...
				TriggerServerEvent('kd-fuel:server:PayForFuel', NozzlePrice)
				DeleteEntity(Nozzle)
				DeleteObject(Nozzle)
				haveNozzle = false
			end
...
CreateThread(function()
	...
	exports['kd-target']:AddTargetBone(bones, {
		options = {
		{
			type = "client",
			event = "kd-fuel:client:SendMenuToServer",
...
RegisterNetEvent('kd-fuel:client:SendMenuToServer', function()
	...
	local ped = PlayerPedId()
		if haveNozzle == true then
		    if CurFuel < 95 then
		     	TriggerServerEvent('kd-fuel:server:OpenMenu', refillCost, inGasStation, false)
	     	else
...
RegisterNetEvent('kd-fuel:client:RefuelVehicle', function(refillCost)
	local ped = PlayerPedId()
	local vehicle = QBCore.Functions.GetClosestVehicle()
	local ped = PlayerPedId()
	...
	local vehicleCoords = GetEntityCoords(vehicle)
			if isCloseVeh() then
				...
				TriggerEvent('kd-fuel:deleteNozzle')
				QBCore.Functions.Progressbar("refuel-car", "Refueling", time, false, true, {
					...
					TriggerServerEvent('kd-fuel:server:PayForFuel', refillCost, GetPlayerServerId(PlayerId()))
					SetFuel(vehicle, 100)
					PlaySound(-1, "5_SEC_WARNING", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
					StopAnimTask(ped, "weapon@w_sp_jerrycan", "fire", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
					TriggerEvent('kd-fuel:createNozzle')
				end, function() -- Cancel
...
RegisterNetEvent('kd-fuel:createNozzle', function (model)
	local ped = PlayerPedId()
...
RegisterNetEvent('kd-fuel:deleteNozzle', function ()
	if haveNozzle == true then
    DeleteEntity(Nozzle)
	DeleteObject(Nozzle)
	haveNozzle = false
...
RegisterNetEvent('kd-fuel:pumpmenu', function ()
        local myMenu = {
                {
                    header = "Fuel Pump",
                    icon = 'fas fa-gas-pump',
                ...
                },
                {
                    ...
                    icon = 'fas fa-x',
                    params = {
                        event = 'kd-menu:client:closeMenu',
                    }
                },
            }
            exports['kd-menu']:openMenu(myMenu)

end)

-- Target Export

exports['kd-target']:AddTargetModel(props, {
	options = {
		{
			type = "client",
			event = "kd-fuel:pumpmenu",
			icon = "fas fa-burn",
			label = "Use Pump",
